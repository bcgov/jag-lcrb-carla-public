    ...
    public partial class SharePointFileManager
    {
        public async Task<FolderItem> CreateFolder2(string relativeUrl)
        {
            ...
            if (!(_statusCode == HttpStatusCode.OK || _statusCode == HttpStatusCode.Created))
            {
                ...
                Console.WriteLine(
                    $"SharePointFileManager - CreateFolderOne - failed - Status: {_statusCode}, relativeUrl: '{relativeUrl}', EscapedRelativeUrl: '{relativeUrl}'"
                );
            ...
            Console.WriteLine(
                $"SharePointFileManager - CreateRootFolder - successfully created folder '{relativeUrl}'"
            );
            Console.WriteLine(
                $"SharePointFileManager - CreateRootFolder - jsonString: {jsonString}"
            );
            FolderItem folderItem = JsonConvert.DeserializeObject<FolderItem>(jsonString);
            return folderItem;
        }

        private char[] GetInvalidCharactersForEntity2(string entityName)
        {
        ...
        public string RemoveInvalidCharacters2(string filename, string entityName = null)
        {
        ...
        private string FixFoldername2(string folderNameOrPath, string entityName = null)
        {
            ...
            if (folderNameOrPath.Contains("/"))
            {
                ...
                var fixedSegments = segments
                    .Select(segment => RemoveInvalidCharacters2(segment, entityName))
                    .ToArray();
        ...
        /// </summary>
        public async Task<List<FolderItem>> SearchForGuidVariant(
            string entityName,
            string guidVariant
        )
        {
            ...
            string query =
                $"web/GetList('/{SharePointHelpers.GetDocumentTemplateUrlPart(entityName)}')/rootFolder/folders?{filter}";
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                try
                {
                    ...
                    foreach (JToken responseResult in responseResults)
                    {
                        ...
                        Console.WriteLine(
                            $"SearchForGuidVariant - Found folder: '{folderItem.Name}'"
                        );
                    }
                }
        ...
        public async Task<List<FolderItem>> FindFolderOne(
            string entityName,
        ...
        {
            Console.WriteLine(
                $"findFolderOne - called with entityName='{entityName}', folderNameSegment='{folderNameSegment}', folderGuidSegment='{folderGuidSegment}'"
            );

            List<FolderItem> allResults = new List<FolderItem>();

            // Try searching for uppercase version
            var uppercaseResults = await SearchForGuidVariant(
                entityName,
                folderGuidSegment.ToUpper()
            );
            ...
            var lowercaseResults = await SearchForGuidVariant(
                entityName,
                folderGuidSegment.ToLower()
            );
            ...
            var foldersWithMaxFileCount = allResults
        ...
        )
        {
            ...
            string sanitizedFolderName = RemoveInvalidCharacters2(rawFolderName);
        ...
        )
        {
            ...
            var matchingFolders = childFolders
                ...
                .ToList();
            Console.WriteLine(
                $"FindFolderThree - Found {matchingFolders.Count} folders matching GUID"
            );

            if (matchingFolders.Count == 0)
            {
                Console.WriteLine(
                    "FindFolderThree - No folders match the GUID, returning empty list"
                );
                return new List<FolderItem>();
            }
        ...
        {
            Console.WriteLine(
                $"GetChildFolders - called with serverRelativeUrl='{serverRelativeUrl}'"
            );
            ...
            {
                Console.WriteLine(
                    "GetChildFolders - SharePoint is not valid, returning empty list"
                );
                return new List<FolderItem>();
            }
        ...
        public async Task<List<FileItem>> GetChildFiles(string serverRelativeUrl)
        {
            Console.WriteLine(
                $"GetChildFiles - called with serverRelativeUrl='{serverRelativeUrl}'"
            );
        ...
        /// </summary>
        private async Task<string> UploadFile2ByFolderId(
            string fileName,
            string folderId,
            byte[] fileData,
            string contentType
        ...
        {
            Console.WriteLine(
                $"UploadFile2ByFolderId - called with folderId='{folderId}', fileName='{fileName}'"
            );
            ...
            Console.WriteLine(
                $"UploadFile2ByFolderId - Using folder ID, URL length: {requestUriString.Length}"
            );
            ...
            {
                Console.WriteLine(
                    $"UploadFile2ByFolderId - Successfully uploaded file '{fileName}' using folder ID"
                );
                return fileName;
            }
            else
            {
                var ex = new SharePointRestException(
                    string.Format(
                        "UploadFile2ByFolderId - Operation returned an invalid status code '{0}'",
                        response.StatusCode
                    )
                );

                Console.WriteLine(
                    $"UploadFile2ByFolderId - Failed to upload file '{fileName}'. Status: {response.StatusCode}, Response: {jsonString}"
                );
        ...
        )
        {
            ...
            {
                Console.WriteLine(
                    "UploadFile2 - serverRelativeUrl is null or empty, returning null"
                );
                return null;
            }
            ...
            if (requestUriString.Length > maxUrlLength)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(folderId))
                    {
                        ...
                        return await UploadFile2ByFolderId(
                            fileName,
                            folderId,
                            fileData,
                            contentType
