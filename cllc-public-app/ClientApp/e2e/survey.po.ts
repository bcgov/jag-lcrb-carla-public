...
export class AppSurveyPage {
  ...
  getSurveyQuestion(surveyConfig, q) {
    const len = surveyConfig.pages.length;
    for (let i = 0; i < len; i++) {
      const p = surveyConfig.pages[i];
      if (p.name == q) {
        return [i, p];
      }
    }
  ...
  async executeSurvey(navPath, surveyConfig) {
    ...
    for (let i = 0; i < len; i++) {
      const myNav = navPath[i];
      const myQ = myNav["q"];
      const myR = myNav["r"];
      const myButton = myNav["button"];
      console.log(`    ${myQ},${myR},${myButton}`);

      const tempP = this.getSurveyQuestion(surveyConfig, myQ);
      const currentSurveyIdx = tempP[0];
      const currentSurveyP = tempP[1];

      // 1. confirm page title
      const currentPageTitle = this.surveyPageTitle();
      ...
      const currentPageButton = this.surveyPageButton(myButton);
