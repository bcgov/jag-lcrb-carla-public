...
export class PoliceSummaryComponent extends FormBase implements OnInit {
  ...
  ngOnInit(): void {
    this.busy = this.specialEventsDataService.getSpecialEventPolice(this.specialEventId).subscribe((application) => {
  ...
  saveTermsAndConditions() {
    this.savedTNC = true;
    this.specialEventsDataService
      .updateSepTermsAndConditions(this.form.value.termsAndConditions, this.specialEventId)
      .subscribe(
        (data: SepTermAndCondtion[]) => {
    ...
    dialogRef
      ...
      .subscribe((acceptApplication) => {
        if (acceptApplication) {
          ...
          this.busy = this.specialEventsDataService.policeApproveSepApplication(this.specialEventId).subscribe(
            () => {
              this.snackBar.open('Reviewed application.', 'Success', {
                duration: 3500,
            ...
            () => {
              this.snackBar.open('Error reviewing the application', 'Fail', {
                duration: 3500,
    ...
    dialogRef
      ...
      .subscribe(([cancelApplication, reason]) => {
        if (cancelApplication) {
          this.busy = this.specialEventsDataService.policeDenySepApplication(this.specialEventId, reason).subscribe(
            () => {
              this.snackBar.open('Denied application.', 'Success', { duration: 3500, panelClass: ['green-snackbar'] });
              this.router.navigate(['/sep/police/my-jobs']);
            },
            () => {
              this.snackBar.open('Error denying the application', 'Fail', {
                duration: 3500,
                panelClass: ['red-snackbar']
              });
  ...
  revoke(): void {
    ...
    dialogRef
      ...
      .subscribe(([cancelApplication, reason]) => {
        if (cancelApplication) {
          this.busy = this.specialEventsDataService.policeCancelSepApplication(this.specialEventId, reason).subscribe(
            () => {
              this.snackBar.open('Cancelled application.', 'Success', {
                duration: 3500,
                panelClass: ['green-snackbar']
              });
              this.router.navigate(['/sep/police/my-jobs']);
  ...
  updateMunicipality(): void {
    this.busy = this.specialEventsDataService
      .policeSetMunicipality(this.specialEventId, this.form.get('sepCity')?.value?.id)
      .subscribe(
        () => {
          this.snackBar.open('Set City.', 'Success', { duration: 3500, panelClass: ['green-snackbar'] });
        },
        () => {
