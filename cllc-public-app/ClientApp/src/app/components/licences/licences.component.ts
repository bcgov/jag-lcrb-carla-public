...
export class LicencesComponent extends FormBase implements OnInit {
  ...
  supportedLicenceTypes = [
    ...
    'Operated - Catering',
    'Deemed - Catering',
    'Transfer in Progress - Catering',
    'Wine Store',
    'Operated - Wine Store',
    'Deemed - Wine Store',
    'Transfer in Progress - Wine Store',
    'Cannabis Retail Store',
    'Marketing',
    'Manufacturer',
    'Operated - Manufacturer',
    'Deemed - Manufacturer',
    'Transfer in Progress - Manufacturer',
    'Licensee Retail Store',
    'Transfer in Progress - Licensee Retail Store',
    'Operated - Licensee Retail Store',
    'Deemed - Licensee Retail Store',
    'UBrew and UVin',
    'Transfer in Progress - UBrew and UVin',
    'Operated - UBrew and UVin',
    'Deemed - UBrew and UVin',
    'S119 CRS Authorization',
    'Transfer in Progress - S119 CRS Authorization'
  ];
  ...
  constructor(
    ...
    featureFlagService.featureOn('PrsEnabled').subscribe((x) => (this.PRS = x));
  }
  ...
  ngOnInit() {
    if (this.liquorThree) {
      ...
      var liquorThree = [
        'Food Primary',
        'Transfer in Progress - Food Primary',
        'Operated - Food Primary',
        'Deemed - Food Primary',
        'Liquor Primary',
        'Transfer in Progress - Liquor Primary',
        'Operated - Liquor Primary',
        'Deemed - Liquor Primary',
        'Liquor Primary Club',
        'Transfer in Progress - Liquor Primary Club',
        'Operated - Liquor Primary Club',
        'Deemed - Liquor Primary Club',
        'Agent',
        'Transfer in Progress - Agent',
        'Operated - Agent',
        'Deemed - Agent'
      ];
  ...
  private displayApplications() {
    const sub = forkJoin([
      ...
      .subscribe(([applications, licenses, operatedLicences, proposedLicences = []]) => {
        ...
        const combinedLicences = [
          ...
          ...licenses.filter(
            (lic) => !(lic.licenceTypeName.includes('Transfer in Progress -') && lic.checklistConclusivelyDeem)
          ),
          ...operatedLicences,
        ...
        if (eventLicenceIds.length > 0) {
          const eventsBusy = this.licenceEventsService
            .getLicenceEventsListBatch(eventLicenceIds, 20)
            .subscribe((allEvents) => {
    ...
    const balanceSub = this.licenceDataService
  ...
  addOrUpdateLicence(licence: ApplicationLicenseSummary) {
    ...
    const licenceIndex = this.licenceMappings[licence.licenceTypeName].findIndex(
      (l) => l.licenseId === licence.licenseId
    );
    if (licenceIndex >= 0) {
