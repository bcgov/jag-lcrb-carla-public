...
export class SepApplicationComponent implements OnInit {
  ...
  async saveToAPI(): Promise<void> {
    ...
    if (appData.id) { // do an update ( the record exists in dynamics)
      let result = await this.sepDataService.updateSepApplication({ ...appData, invoiceTrigger: true } as SepApplication, appData.id)
        .toPromise();
      if (result.localId) {
        await this.db.applications.update(result.localId, result);
      }
    } else {
      let result = await this.sepDataService.createSepApplication({ ...appData, invoiceTrigger: true } as SepApplication)
        .toPromise();
      if (result.localId) {
        await this.db.applications.update(result.localId, result);
