...
namespace Gov.Lclb.Cllb.Services.FileManager
{
    ...
    {
        const int MAX_TOTAL_LENGTH = 167; // Tested against SharePoint to determine max path length that can be reliably used
        ...
        public async Task<EnsureFolderPathReply> _ensureFolderPath(EnsureFolderPathRequest request)
        {
            ...
            var documentListTitle = SharePointHelpers.GetDocumentListTitle(request.EntityName);
            var documentTemplateUrlPart = SharePointHelpers.GetDocumentTemplateUrlPart(request.EntityName);
        ...
        public async Task<FolderItem> createFolderOne(EnsureFolderPathRequest request)
        {
            ...
            var createdFolder = await CreateFolder(
                request.EntityName,
                SharePointHelpers.GetDocumentTemplateUrlPart(request.EntityName),
                segment.FolderNameSegment,
                segment.FolderGuidSegment
            );
        ...
        public async Task<FolderItem> CreateFolder(string entityName, string parentRelativePath, string folderName)
        {
            ...
            var sanitizedFolderName = _sharePointFileManager.RemoveInvalidCharacters2(folderName, entityName);

            string relativeUrl = _sharePointFileManager.EscapeApostrophe($"{parentRelativePath}/{sanitizedFolderName}");

            return await _sharePointFileManager.CreateFolder2(relativeUrl);
        }
