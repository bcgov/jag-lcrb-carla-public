...
namespace bdd_tests
{
    public abstract class TestBaseWorker : Feature
    {
        ...
        public void CarlaLoginWorkerNoTerms()
        {
            ...
            var test_start = configuration["test_start_worker"];
        ...
        public void CarlaLoginWorker()
        {
            ...
            var test_start = "bcservice/token/AT" + DateTime.Now.Ticks + random.Next(0, 999);
            ...
            */
            ...
            var uiCheckBox1 = ngDriver.FindElement(By.CssSelector("mat-checkbox#mat-checkbox-1"));
            uiCheckBox1.Click();

            var uiCheckBox2 = ngDriver.FindElement(By.CssSelector("mat-checkbox#mat-checkbox-2"));
            uiCheckBox2.Click();

            var continueButton = ngDriver.FindElement(By.CssSelector(".buttons .btn-primary"));
        ...
        {
            var deleteAccountURL = $"{baseUri}api/accounts/delete/current";
            var script = $"fetch(\"{deleteAccountURL}\", {{method: \"POST\", body: {{}}}})";
        ...
        public void Step1NotCompleted()
        {
            ...
            var submitButton = ngDriver.FindElement(By.CssSelector("span .btn-primary"));
            submitButton.Click();
        }
        ...
        public void Step1ErrorMessagesDisplayed()
        {
            ...
            Assert.True(ngDriver
                .FindElement(By.XPath("//body[contains(.,' The city and country of your birth is required. ')]"))
                .Displayed);

            // check that missing BCID/DL error messages are thrown
            Assert.True(ngDriver
                .FindElement(By.XPath(
                    "//body[contains(.,' You must enter either a BCID number or a driver’s licence number. ')]"))
                .Displayed);
            Assert.True(ngDriver
                .FindElement(By.XPath("//body[contains(.,' Format is 9 digits for BCID and 9 digits for driver')]"))
                .Displayed);
            ...
            Assert.True(ngDriver.FindElement(By.XPath("//body[contains(.,'(incl. area code) example: 1234567890')]"))
                .Displayed);
            Assert.True(ngDriver.FindElement(By.XPath("//body[contains(.,'This field cannot contain letters.')]"))
                .Displayed);
            Assert.True(ngDriver
                .FindElement(By.XPath(
                    "//body[contains(.,'Please provide a 10-digit phone number without a country code, spaces or special characters.*')]"))
                .Displayed);

            // check that missing email error messages are thrown
            Assert.True(ngDriver
                .FindElement(By.XPath("//body[contains(.,'Please include an “@” in the email address.')]")).Displayed);
            Assert.True(ngDriver
                .FindElement(By.XPath(
                    "//body[contains(.,'Your address must include a domain at the end such as .com, .ca, or .net.')]"))
                .Displayed);

            // check that missing postal code error message is thrown
            Assert.True(ngDriver
                .FindElement(By.XPath(
                    "//body[contains(.,'Your postal / zip code should be in one of the following formats:')]"))
                .Displayed);
            ...
            Assert.True(ngDriver
                .FindElement(By.XPath("//body[contains(.,'Some required fields have not been completed')]")).Displayed);
        }
        ...
        public void Step2NotCompleted()
        {
            ...
            var submitButton = ngDriver.FindElement(By.CssSelector("button.btn-primary"));
            submitButton.Click();
        }
        ...
        public void Step2ErrorMessagesDisplayed()
        {
            ...
            Assert.True(ngDriver
                .FindElement(By.XPath(
                    "//body[contains(.,' Please provide your consent to proceed -- a security screening is required in order to become a qualified cannabis worker. ')]"))
                .Displayed);

            // check that missing required fields error message is thrown
            Assert.True(ngDriver
                .FindElement(
                    By.XPath("//body[contains(.,' Please make sure that you have completed all required fields. ')]"))
                .Displayed);
        }
